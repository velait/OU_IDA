...
transformed parameters {
  ...
  // real<lower=0, upper=1> inv_gamma_mode = lambda_beta/(lambda_alpha + 1);
...
model {
  ...
  // lambda ~ inv_gamma(10, lambda_beta);
  // lambda_raw ~ normal(0, 1);
  
  
  // lambda ~ inv_gamma(3, 2);
  ...
  sigma ~ inv_gamma(sigma_mu, sigma_sigma);
  
  // student_df ~ normal(0, 100);
  
  //hyper priors
  lambda_alpha ~ normal(10, 10);
  lambda_beta ~ normal(10, 10);
  
  
  // inv_gamma_mode ~ normal(0.5, 0.5);
  // inv_gamma_variance ~ normal(0, 1);
  
  mu_mu ~ normal(0, 5);
  mu_sigma ~ normal(0, 5);
  
  sigma_mu ~ normal(10, 10);
  sigma_sigma ~ normal(10, 10);
